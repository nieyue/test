...
var myUtils = {
	...
	/*
	*字符串指定查找字符串的个数
	*str 主字符串 substr 要查找的字符串
	*/
findStringNum:function (str,substr) {

var regex = new RegExp(substr, 'g'); // 使用g表示整个字符串都要匹配
...
var count = !result ? 0 : result.length;
  return count;
	 ...
	 */
	myClickRotate:function(clickObj,rotateObj,deg){
		var isClick=false;
		var co=document.querySelector(clickObj);
		var ro=document.querySelector(rotateObj);
		if(!co){return;};
		var origindeg=deg;
		if(!deg){
			origindeg='180';
		...
		co.onclick=function(){
    		...
    		$this.style.transform='rotate('+deg+'deg)';
			 ...
			 deg=parseInt(origindeg)+parseInt(deg);
		};
	},
	/**
